...
def bigrams_count_to_probabilities(
    ...
    suma_filas = torch.sum(smoothed_counts,1) 
    suma_filas = suma_filas.unsqueeze(1)
    probabilities = smoothed_counts / suma_filas
...
def calculate_neg_mean_log_likelihood(
    ...
    for word in words:
        total_log_likelihood += calculate_log_likelihood(word=word, bigram_probabilities=bigram_probabilities, char_to_index=char_to_index, start_token=start_token, end_token=end_token)

    # Calculate and return the negative mean log likelihood
    mean_log_likelihood: float = total_log_likelihood/(-1 * len(words))
...
def sample_next_character(
    ...
    next_char_index: int = torch.multinomial(input=current_probs, num_samples=1, replacement=False)
...
def generate_name(
    ...
    # if len(generated_name) == max_length, one more iteration will be done (because start token is being counted too)
    while current_char != end_token:
        generated_name += sample_next_character(current_char_index = char_to_idx[current_char],probability_distribution = bigram_probabilities, idx_to_char = idx_to_char)
...
def calculate_log_likelihood(
    ...
    for i in range(len(processed_word)-1):
        ...
        char1 = char_to_index[processed_word[i]]
        char2 = char_to_index[char2]
        log_likelihood += torch.log(bigram_probabilities[char1,char2])
